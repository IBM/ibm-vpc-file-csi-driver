...
// CapacityRoundoff determines the minimum capacity (GiB) that satisfies a
// requested IOPS value for a given volume profile's bands.
type CapacityRoundoff interface {
	// GetMinCapacityForIops returns the minimum capacity in GiB whose
	// IOPSMax >= requestedIops. Returns an error if no band covers the IOPS.
...
// of volume profile bands. Returns an error if bands is nil or empty.
