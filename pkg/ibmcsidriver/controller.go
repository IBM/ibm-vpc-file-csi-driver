...
type CSIControllerServer struct {
	...
	ProfileBands []provider.VolumeProfileBand
...
func (csiCS *CSIControllerServer) CreateVolume(ctx context.Context, req *csi.CreateVolumeRequest) (*csi.CreateVolumeResponse, error) {
	...
	requestedVolume, err := getVolumeParameters(ctxLogger, req, csiCS.CSIProvider.GetConfig(), csiCS.ProfileBands)
...
func (csiCS *CSIControllerServer) ListSnapshots(ctx context.Context, req *csi.ListSnapshotsRequest) (*csi.ListSnapshotsResponse, error) {
	...
	if len(snapshotID) != 0 {
		...
		if len(snapID) != 0 {
			snapshot, _ := session.GetSnapshot(snapID, volumeID) // #nosec G104: Errors are intentionally not handled as we are checking for the presence of the expected output only.
			if snapshot == nil {
