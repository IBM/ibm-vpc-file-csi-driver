...
func TestValidateNFSMountOptions(t *testing.T) {
	testCases := []struct {
		...
		{
			name:          "Valid mount options with vers and proto",
			options:       []string{"vers=4.1", "proto=tcp"},
			expectedError: "",
		},
		{
			name:          "Valid mount options with nfsvers and proto",
			options:       []string{"nfsvers=4.1", "proto=tcp", "rsize=1048576"},
			expectedError: "",
		},
		{
		...
		},
		{
			...
			options:       []string{"vers=4.1", "proto=tcp", "rsize=1048576", "wsize=1048576", "hard", "timeo=600"},
			expectedError: "",
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			err := validateNFSMountOptions(tc.options)
			if tc.expectedError == "" {
...
func TestConfigureEncryptionInTransit(t *testing.T) {
	...
	testCases := []struct {
		...
		volumeCapabilities     []*csi.VolumeCapability
		expectedTransitEncrypt string
		expectedError          string
	}{
		{
			name: "DP2 profile with EIT enabled - should set IPSEC",
			volume: &provider.Volume{
				VPCVolume: provider.VPCVolume{
					Profile: &provider.Profile{Name: DP2Profile},
					VPCFileVolume: provider.VPCFileVolume{
						TransitEncryption: EncryptionTransitMode,
					},
				},
			},
			volumeCapabilities: []*csi.VolumeCapability{
			...
			expectedTransitEncrypt: IPSEC,
			expectedError:          "",
		},
		{
			name: "RFS profile with EIT enabled and valid mount options - should set STUNNEL",
			volume: &provider.Volume{
				VPCVolume: provider.VPCVolume{
		...
		{
			name: "RFS profile with EIT enabled but invalid fsType",
			volume: &provider.Volume{
				VPCVolume: provider.VPCVolume{
					Profile: &provider.Profile{Name: RFSProfile},
					VPCFileVolume: provider.VPCFileVolume{
			...
			expectedTransitEncrypt: STUNNEL,
			expectedError:          "invalid fsType 'nfs' for RFS profile with encryption-in-transit, must be 'nfs4'",
		},
		{
			name: "RFS profile with EIT enabled but missing mount options",
			volume: &provider.Volume{
				VPCVolume: provider.VPCVolume{
					Profile: &provider.Profile{Name: RFSProfile},
					VPCFileVolume: provider.VPCFileVolume{
		...
		},
		{
			...
			expectedTransitEncrypt: NONE,
			expectedError:          "",
		},
		{
			name: "RFS profile with EIT disabled - should set NONE",
			volume: &provider.Volume{
				VPCVolume: provider.VPCVolume{
					Profile: &provider.Profile{Name: RFSProfile},
					VPCFileVolume: provider.VPCFileVolume{
		...
		{
			name: "RFS profile with EIT and empty fsType - should succeed",
			volume: &provider.Volume{
				VPCVolume: provider.VPCVolume{
					Profile: &provider.Profile{Name: RFSProfile},
					VPCFileVolume: provider.VPCFileVolume{
		...
		{
			name: "RFS profile with EIT and nil mount capability",
			volume: &provider.Volume{
				VPCVolume: provider.VPCVolume{
					Profile: &provider.Profile{Name: RFSProfile},
					VPCFileVolume: provider.VPCFileVolume{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			err := configureEncryptionInTransit(logger, tc.volume, tc.volumeCapabilities)

			if tc.expectedError == "" {
				assert.NoError(t, err)
				assert.Equal(t, tc.expectedTransitEncrypt, tc.volume.VPCVolume.TransitEncryption)
			} else {
				assert.Error(t, err)
				assert.Contains(t, err.Error(), tc.expectedError)
			}
